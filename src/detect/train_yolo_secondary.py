...
# NOTE: need modify settings.json to train YOLO correctly
...
def train():
    # 固定参数
    CONFIG = {
        'data': str(ROOT_DIR / "configs/detect/train_yolo_secondary.yaml"),        # config file
        'epochs': 100,              # 训练轮数
        'imgsz': 640,              # 图像大小
        'batch': 16,               # 批次大小
        'device': 0 if torch.cuda.is_available() else 'cpu',  # 使用GPU如果可用
        'project': str(ROOT_DIR / "outputs/0215/yolo_secondary/runs/train"),    # 保存目录
        'name': 'exp',             # 实验名称
    }
    
    print("Start Training...")
    print(f"Use GPU/CPU: {'GPU' if torch.cuda.is_available() else 'CPU'}")
