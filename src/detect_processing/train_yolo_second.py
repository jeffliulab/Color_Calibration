...
def update_ultralytics_settings():
    ...
    settings.update({
        "datasets_dir": str(ROOT_DIR / "data/train/detect/yolo_second_0214/"),  # Dataset storage path
        "weights_dir": str(ROOT_DIR / f"weights/detect/yolo_second/{unique_date}/"),  # Model weights storage path
        "runs_dir": str(ROOT_DIR / f"outputs/runs/detect/yolo_second/{unique_date}/")  # Experiment logs storage path
    })
...
def train():
    # 固定参数
    CONFIG = {
        'data': str(ROOT_DIR / "configs/detect/train_yolo_second.yaml"),        # 数据配置文件
        'epochs': 100,              # 训练轮数
        'imgsz': 640,              # 图像大小
        'batch': 16,               # 批次大小
        'device': 0 if torch.cuda.is_available() else 'cpu',  # 使用GPU如果可用
        'project': str(ROOT_DIR / f"outputs/runs/detect/yolo_second/{unique_date}/"),    # auto save in YOLO
        'name': 'exp',             # 实验名称
    }
